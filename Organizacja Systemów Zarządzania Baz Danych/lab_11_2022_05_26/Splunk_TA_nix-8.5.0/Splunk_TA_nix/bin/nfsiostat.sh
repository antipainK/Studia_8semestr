...
if [ "$KERNEL" = "Linux" ] ; then
    ...
    then
        ...
        if [ "$OS_RELEASE" = "\"18.04\"" ] || [ "$OS_RELEASE" = "\"20.04\"" ] ; then # Ubuntu 18.04 and 20.04
            # shellcheck disable=SC2016
            FORMAT='{
                if (NR%10==2){
                echo "device"
    ...
    # For CentOS and RHEL
        ...
        if [ -e $OS_FILE ] && (awk -F'=' '/ID=/ {print $2}' $OS_FILE | grep -q rhel) && (awk -F'=' '/VERSION_ID=/ {print $2}' $OS_FILE | grep -Eq 8.5\|8.4\|8.3);
        then
            # shellcheck disable=SC2016
            FORMAT='{
                if (NR%10==2){
